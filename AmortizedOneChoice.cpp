...
AmortizedOneChoice::~AmortizedOneChoice() {

}

void AmortizedOneChoice::update(OP op, string keyword, int ind, bool setup) {
    if (!setup) {
...
vector<int> AmortizedOneChoice::search(string keyword) {
    L->TotalCacheTime = 0;
    L->searchTime = 0;
    L->totalCommunication = 0;
    vector<int> finalRes;
...
prf_type AmortizedOneChoice::bitwiseXOR(int input1, int op, prf_type input2) {
    prf_type result;
...
prf_type AmortizedOneChoice::bitwiseXOR(prf_type input1, prf_type input2) {
    prf_type result;
...
double AmortizedOneChoice::getTotalSearchCommSize() const {
    return totalSearchCommSize;
}

double AmortizedOneChoice::getTotalUpdateCommSize() const {
    return totalUpdateCommSize;
}

void AmortizedOneChoice::endSetup() {
    if (setup) {
...
void AmortizedOneChoice::beginSetup() {
    setup = true;
    //setup = false;
...
bool AmortizedOneChoice::setupFromFile(string filename) {
    return false;
}