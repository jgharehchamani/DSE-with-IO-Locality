...
AmortizedTwoChoice::~AmortizedTwoChoice() {

}

void AmortizedTwoChoice::update(OP op, string keyword, int ind, bool setup) {
    if (!setup) {
        L->TotalCacheTime = 0;
        L->searchTime = 0;
        L->totalCommunication = 0;
        L->oneChoiceServer->storage->cacheTime = 0;
        L->twoChoiceServer->keywordCounters->cacheTime = 0;
        L->twoChoiceServer->storage->cacheTime = 0;
...
vector<int> AmortizedTwoChoice::search(string keyword) {
    L->TotalCacheTime = 0;
    L->searchTime = 0;
    L->totalCommunication = 0;
    vector<int> finalRes;
...
prf_type AmortizedTwoChoice::bitwiseXOR(int input1, int op, prf_type input2) {
    prf_type result;
...
prf_type AmortizedTwoChoice::bitwiseXOR(prf_type input1, prf_type input2) {
    prf_type result;
...
double AmortizedTwoChoice::getTotalSearchCommSize() const {
    return totalSearchCommSize;
}

double AmortizedTwoChoice::getTotalUpdateCommSize() const {
    return totalUpdateCommSize;
}

void AmortizedTwoChoice::endSetup() {
    if (setup) {
...
void AmortizedTwoChoice::beginSetup() {
    setup = true;
    //setup = false;
...
bool AmortizedTwoChoice::setupFromFile(string filename) {
    return false;
}